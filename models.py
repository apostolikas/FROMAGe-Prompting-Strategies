    ...
    with torch.no_grad():  # no tracking history
      ...
      for i in range(max_len):
        ...
        if content_free_logits!= [] and i <= 4:
          logits /= nn.Softmax(dim=-1)(content_free_logits[i])
...
class Fromage(nn.Module):
  ...
  def generate_with_content_free(self, prompts: List, num_words: int = 0, ret_scale_factor: float = 1.0, top_p: float = 1.0, temperature: float = 0.0,
    max_num_rets: int = 1, max_img_per_ret: int = 1, id:int=1,constrained_ids:Tensor=None):
    """
    ...
    content_free_logits = [( nn.Softmax(dim=-1)(black_content_free_logits[i])+
                            nn.Softmax(dim=-1)(white_content_free_logits[i]) ) /2 for i in range(len(black_content_free_logits))]
    # content_free_logits = []
    # # save
